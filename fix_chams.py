...
interactive_section = """struct InteractiveSectionView: View {
    let title: String
    let items: [String]
...
# Actually replace the text `struct MockSectionView...` block
mock_section_pattern = re.compile(r'struct MockSectionView: View \{.*?(?=\nstruct CustomSettingsView: View)', re.DOTALL)
text = mock_section_pattern.sub(interactive_section + '\n\n', text)

# Replace instances of MockSectionView( with InteractiveSectionView(
text = text.replace('MockSectionView(', 'InteractiveSectionView(')

# Update CustomSettingsView
settings_pattern = re.compile(r'struct CustomSettingsView: View \{.*?(?=\nstruct SettingsRow: View)', re.DOTALL)

new_settings = """struct CustomSettingsView: View {
...
"""
text = settings_pattern.sub(new_settings, text)

with open('ThreeOneOSFive/ContentView.swift', 'w', encoding='utf-8') as f:
    f.write(text)
print("Updated Chams, App Name, Interactive Toggles, and Settings.")
